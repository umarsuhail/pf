...
export function CardPortal({
  ...
  // Drives the entry portal's ParticleLogo form-in/disperse-out — derived
  // from the same scroll-reveal math below rather than a plain viewport
  // IntersectionObserver, since this card's visibility is CSS opacity/
  // scale, not geometry (it stays inside the sticky flight container at
  // all times).
      ...
      <div className="absolute inset-0 overflow-hidden rounded-2xl [clip-path:inset(0_round_1rem)] lg:rounded-3xl lg:[clip-path:inset(0_round_1.5rem)]">
        ...
        >
          ...
          {isEntry ? (
            ...
            >
              {/* The background image behind this (see Space/mix-blend-screen
                 above) is a faint decorative texture, not a standalone mark —
                 this slot carries the real logo: the particle-formed "US"
                 mark normally, swapping to the astronaut illustration while
                 the bio is expanded. */}
