...
export default function HoloChat() {
  ...
  const sendMessage = async (override?: string) => {
    ...
    const history = messages.slice(-8).map((m) => ({ role: m.role, message: m.text.slice(0, 700) }));
              ...
              <div key={index} className="max-w-[94%] self-start">
                ...
                <div className="rounded-2xl rounded-tl-sm border border-cyan-100/10 bg-[linear-gradient(135deg,rgba(8,47,73,0.62),rgba(15,23,42,0.74))] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_12px_30px_rgba(0,0,0,0.16)]">
                  ...
                  {!!message.sources?.length && (
                    <div className="mt-3 flex flex-wrap gap-2 border-t border-cyan-100/10 pt-2 text-xs text-cyan-200">
                      <span className="w-full text-cyan-200/60">Reference passages</span>
                      {message.sources.filter((source, i, all) => all.findIndex(other => other.url === source.url) === i).map(source => (
                        <a key={source.id} href={source.url} target="_blank" rel="noopener noreferrer" className="underline underline-offset-2 hover:text-white">
                          {source.url === '/resume.tex' ? 'Resume source' : source.title}
                        </a>
                      ))}
                    </div>
                  )}
